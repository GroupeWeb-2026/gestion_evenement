...
export default function AdminSpeakersPage() {
  ...
  const deleteSpeaker = async (id: string) => {
    if (
      !confirm(
        "Retirer ce speaker ? (les sessions existantes garderont son nom)",
      )
    )
      return;
  ...
  const restoreSpeaker = async (id: string) => {
    if (
      !confirm(
        "Restaurer ce speaker ? Il réapparaîtra dans les options des sessions",
      )
    )
      return;
          ...
          <table className="w-full">
            ...
            <tbody>
              {speakers.map((speaker) => (
                <tr
                  key={speaker.id}
                  className={`border-b hover:bg-gray-50 ${speaker.isDeleted ? "opacity-70 bg-gray-100" : ""}`}
                >
                  ...
                  <td className="p-4">
                    {speaker.isDeleted ? (
                      <button
                        onClick={() => restoreSpeaker(speaker.id)}
